...
module.exports = {
    async handler(chatUpdate) {
        ...
        let khosy = chatUpdate.messages[0]
        await conn.readMessages([khosy.key])
        if (!khosy.message) return
        if (!m) return
        ...
        global.namabot = 'Nᴇᴢʜᴀ-Bᴏᴛ'
        global.wm = namabot + ' • Bʏ Tᴀᴋᴀsʜɪシ︎'
        ...
        try {
            ...
            for (let name in global.plugins) {
                ...
                if ((usedPrefix = (match[0] || '')[0])) {
                    ...
                    if (xp > 200) m.reply('Ngecit Anjir -_-') // Hehehe
                    else m.exp += xp
    ...
    async participantsUpdate({ id, participants, action }) {
        ...
        switch (action) {
                ...
                if (chat.welcome) {
                    ...
                    for (let user of participants) {
                        ...
                        try {
                            ...
                            await this.send3TemplateButtonImg(id, action === 'add' ? pp : pp, text, wm, action === 'add' ? 'Selamat datang' : 'Sampai jumpa', action === 'add' ? '.intro' : 'Takashi')
                        }
                    }
                }
                break
            ...
            case 'promote':
                text = (chat.sPromote || this.spromote || conn.spromote || '@user ```Selamat Udah Naik Jabatan```')
            case 'demote':
                if (!text) text = (chat.sDemote || this.sdemote || conn.sdemote || '@user ```Sabar yah bang dah di turunin```')
...
global.dfail = async (type, m, conn) => {
    let msg = {
        ...
        premium: 'Perintah ini hanya Khusus Premium',
        group: `Perintah ini hanya dapat digunakan di grup!`,
        private: 'Perintah ini hanya dapat digunakan di Chat Pribadi!',
        admin: 'Perintah ini Khusus admin',
        botAdmin: 'Bot nya jadiin admin dulu',
        unreg: 'Daftar dulu untuk menggunakan fitur ini cara nya:\n\n*#daftar nama.umur*\n\nContoh: *#daftar Namalu.16*',
        nsfw: `NSFW tidak aktif, Yahaha mau cari apaan lu`,
        rpg: `RPG tidak aktif, Silahkan hubungi owner untuk mengaktifkan nya`,
