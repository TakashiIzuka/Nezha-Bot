...
if(!img) throw m.reply(`balas gambar/video/stiker dengan perintah .stiker`)
    await conn.sendStimg(m.chat , img, m, {packname: 'Nᴇᴢʜᴀ-Bᴏᴛ', author: `${conn.getName(m.sender)}`})
    await delay(1000)
    await conn.sendMessage(m.chat, { delete: key })
    } catch (err) { console.log(err); throw 'Gagal!, Balas Gambar/video dengan caption .stiker'}
}
