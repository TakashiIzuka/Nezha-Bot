...
handler.all = async function (m) {
	...
	try {
		...
		global.kontak2 = [
         [owner[0], await this.getName(owner[0] + '@s.whatsapp.net'), 'Yg recode ʙᴏᴛ', 'falakhiln@gmail.com', true],
         [owner[1], await this.getName(owner[1] + '@s.whatsapp.net'), 'Yg run ʙᴏᴛ', 'falakhiln@gmail.com', true],
         [owner[2], await this.getName(owner[2] + '@s.whatsapp.net'), 'Yg support ʙᴏᴛ', 'falakhiln@gmail.com', true]// Kalo mau di tambah tinggal copy 1baris ini di tempel di bawahnya trs di edit dikit!
        ]
...
function ucapan() {
    ...
    let res = "Mat Malam Bang 🌙"
    if (time >= 4) {
        res = "Mat Pagi Bang 🌄"
    }
    if (time > 10) {
        res = "Mat Siang Bang ☀️"
    }
    if (time >= 15) {
        res = "Mat Sore Bang 🌅"
    }
